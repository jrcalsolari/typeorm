...
const PostWithOptions_1 = require("./entity/PostWithOptions");
const test_utils_1 = require("../../../../utils/test-utils");
const PostWithoutTypes_1 = require("./entity/PostWithoutTypes");
...
describe("database schema > column types > postgres", () => {
    ...
    it("all types should work correctly - persist and hydrate when options are specified on columns", () => Promise.all(connections.map(async (connection) => {
        const postRepository = connection.getRepository(PostWithOptions_1.PostWithOptions);
        const queryRunner = connection.createQueryRunner();
        const table = await queryRunner.getTable("post_with_options");
        await queryRunner.release();
        const post = new PostWithOptions_1.PostWithOptions();
        post.id = 1;
        post.numeric = "50.00";
        post.decimal = "50.00";
        post.char = "AAA";
    ...
    it("all types should work correctly - persist and hydrate when types are not specified on columns", () => Promise.all(connections.map(async (connection) => {
        const postRepository = connection.getRepository(PostWithoutTypes_1.PostWithoutTypes);
        const queryRunner = connection.createQueryRunner();
        const table = await queryRunner.getTable("post_without_types");
        await queryRunner.release();
        const post = new PostWithoutTypes_1.PostWithoutTypes();
        post.id = 1;
        post.name = "Post";
        post.bit = true;
