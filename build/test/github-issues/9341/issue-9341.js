...
describe('github issues > #9341 "bigNumberStrings:false" is not working for postgres', () => {
    ...
    it("should fetch big int as number not string when using parseInt8=true", async () => {
        for (const connection of connections) {
            ...
            (0, chai_1.expect)(typeof result?.big_int).to.eq("number");
            (0, chai_1.expect)(result?.big_int).to.eq(Number.MAX_SAFE_INTEGER);
            // big decimal should remain string, only int8 is parsed
            (0, chai_1.expect)(typeof result?.big_decimal).to.eq("string");
            (0, chai_1.expect)(result?.big_decimal).to.eq("1.23456789");
