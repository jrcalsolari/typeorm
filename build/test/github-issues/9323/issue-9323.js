...
describe("github issues > #9323 Add new VirtualColumn decorator feature", () => {
    ...
    it("should be able to save and find sub-select data in the database", () => Promise.all(connections.map(async (connection) => {
        ...
        (0, chai_1.expect)(usersUnderCompany?.totalEmployeesCount).to.eq(4);
        ...
        (0, chai_1.expect)(employee1TimesheetFound?.totalActvityHours).to.eq(6);
        ...
        (0, chai_1.expect)(usersUnderCompanyListOne?.totalEmployeesCount).to.eq(4);
        const employee1TimesheetListOneFound = usersUnderCompanyListOne?.employees
            .find((e) => e.name === employee1.name)
            ?.timesheets.find((ts) => ts.id === employee1TimeSheet.id);
        (0, chai_1.expect)(employee1TimesheetListOneFound?.totalActvityHours).to.eq(6);
    })));
    it("should be able to save and find sub-select data in the database (with query builder)", () => Promise.all(connections.map(async (connection) => {
        ...
        const foundEmployeeTimeSheet = foundEmployee?.timesheets.find((t) => t.id === employee1TimeSheet.id);
        (0, chai_1.expect)(foundEmployeeTimeSheet?.totalActvityHours).to.eq(6);
    })));
});
